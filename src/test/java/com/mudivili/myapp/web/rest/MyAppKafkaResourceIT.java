...
import com.mudivili.myapp.config.EmbeddedKafka;
import com.mudivili.myapp.config.KafkaSseConsumer;
import com.mudivili.myapp.config.KafkaSseProducer;
...
class MyAppKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/my-app-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/my-app-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/my-app-kafka/unregister"));
                return;
            }
        }
